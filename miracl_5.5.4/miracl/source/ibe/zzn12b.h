/*
 *    MIRACL  C++ Header file ZZn12b.h
 *
 *    AUTHOR  : M. Scott
 *
 *    NOTE:   : Must be used in conjunction with big.cpp, zzn.cpp and zzn3.cpp and zzn6.cpp, 
 *				uses 1-3-6-12 towering, and assumes 12|p-1
...
public:
	...
	ZZn12& operator*=(const ZZn& x) { a*=x; b*=x; unitary=FALSE; return *this;}
    ZZn12& operator*=(int x) { a*=x; b*=x; unitary=FALSE; return *this;}
    ZZn12& operator/=(const ZZn12&); 
    ZZn12& operator/=(const ZZn6&);

	friend ZZn12 tx(const ZZn12&);
	friend ZZn12 tx2(const ZZn12&);
	friend ZZn12 tx4(const ZZn12&);
	friend ZZn12 tx8(const ZZn12&);
