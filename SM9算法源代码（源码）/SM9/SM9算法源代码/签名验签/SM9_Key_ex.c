...
Description: convert 128 bytes into ecn2Calls: MIRACL functions
...
void LinkCharZzn12(unsigned char *message, int len, zzn12 w, unsigned char *Z, int Zlen)
{
	...
	redc(w.b.a.b, tmp); big_to_bytes(BNLEN, tmp, Z + len + BNLEN * 6, 1); redc(w.b.a.a, tmp); big_to_bytes(BNLEN, tmp, Z + len + BNLEN * 7, 1);
...
int Test_Point(epoint* point)
{
	...
	if (point_at_infinity(buf) == FALSE)return 1;
	return 0;
}
/****************************************************************
	...
	g2, zzn12 g3, int klen, unsigned char K[])
{
	...
	LinkCharZzn12(Z, 0, g1, Z + IDALen + IDBLen + BNLEN * 4, BNLEN * 12); LinkCharZzn12(Z, 0, g2, Z + IDALen + IDBLen + BNLEN * 16, BNLEN * 12);
	...
	*RA, epoint *RB, zzn12 g1, zzn12 g2, zzn12 g3, unsigned char hash[])
{
	...
	memcpy(Z + BNLEN * 24, IDA, IDALen); memcpy(Z + BNLEN * 24 + IDALen, IDB, IDBLen);
...
int SM9_H1(unsigned char Z[], int Zlen, big n, big h1)
{
	...
	decr(n, 1, n1);
	ZH = (char *)malloc(sizeof(char)*(ZHlen + 1)); if (ZH == NULL) return SM9_ASK_MEMORY_ERR;
	memcpy(ZH + 1, Z, Zlen);
	ZH[0] = 0x01;
...
int SM9_Init()
{
	...
	X.a = mirvar(0); X.b = mirvar(0);
	P2.x.a = mirvar(0); P2.x.b = mirvar(0); P2.y.a = mirvar(0); P2.y.b = mirvar(0);
...
SA which responder B might verifies(Step A5-A7)Calls: MIRACL functions,SM9_H1,Test_Point,ecap(),member(),zzn12_init
	...
	char SA[])
{
	...
	bytes_to_big(BNLEN, Ppub, x); bytes_to_big(BNLEN, Ppub + BNLEN, y); bytes_to_big(BNLEN, randA, rA);
...
/****************************************************************
Function: SM9_SelfCheckDescription: SM9 self check
...
int main()
{
	...
	{ 0x0F,0xE8,0xEA,0xB3,0x95,0x19,0x9B,0x56,0xBF,0x1D,0x75,0xBD,0x2C,0xD6,0x10,0xB6,0x42,0x4F,0x08,0xD1,0x09,0x29,0x22,0xC5,0x88,0x2B,0x52,0xDC,0xD6,0xCA,0x83,0x2A,
	...
	int tmp, i;
	mip = mirsys(1000, 16);
	mip->IOBASE = 16; x = mirvar(0); y = mirvar(0); ke = mirvar(0);
	...
	//printf("%d\n", memcmp(deB, std_deB, 128));
	if (memcmp(deA, std_deA, 128) != -1)
		return SM9_GEPRI_ERR;
	
	if (memcmp(deB, std_deB, 128) != -1)
		return SM9_GEPRI_ERR;
	...
	if (tmp != 0) return tmp;
	epoint_get(RB, x, y);
	big_to_bytes(BNLEN, x, xy, 1); big_to_bytes(BNLEN, y, xy + BNLEN, 1); if (memcmp(xy, std_RB, BNLEN * 2) != 0)
		return SM9_ERR_RB;
