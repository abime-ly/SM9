...
{
    if (zzn2_compare(&x->a, &y->a)&&zzn2_compare(&x->b,&y->b)) 
		return TRUE;
    return FALSE;
}


void zzn4_negate( zzn4 *x,zzn4 *w)
{
    zzn4_copy(x,w);
    zzn2_negate(&w->a,&w->a);
    zzn2_negate(&w->b,&w->b);
...
void zzn4_tx3( zzn4 *x, zzn2 *y,zzn4 *w)
{
	zzn2_mul(&x->a,y,&w->a);
	zzn2_mul(&x->b,y,&w->b);
	w->unitary = FALSE;
...
void zzn4_add( zzn4 *x,zzn4 *y,zzn4 *w)
{
    zzn2_add( &x->a,&y->a,&w->a);
    zzn2_add( &x->b,&y->b,&w->b);
    w->unitary = FALSE;
}
void zzn4_sub( zzn4 *x,zzn4 *y,zzn4 *w)
{
    zzn2_sub( &x->a,&y->a,&w->a);
    zzn2_sub( &x->b,&y->b,&w->b);
    w->unitary = FALSE;
}


void zzn4_powq( zzn2 *m, zzn4 *w)
{ 
    zzn2_conj( &w->a,&w->a);
    zzn2_conj( &w->b,&w->b);
    zzn2_mul( &w->b, m, &w->b);
...
zzn12_copy(&x,z);
if( zzn4_compare(&x.a,&y.a) && zzn4_compare(&x.b,&y.b)&&zzn4_compare(&x.c,&y.c))
{
if (x.unitary==TRUE)
{
...
BOOL invert_it;
zzn12 res;
zero1=mirvar(0);tmp=mirvar(0);tmp1=mirvar(0);
zzn12_init(&res);
copy(k,tmp1);
invert_it=FALSE;
if(mr_compare(tmp1,zero1)==0)
{
tmp=get_mip()->one;nres(tmp,res.a.a.a);zero(res.a.a.b);  zero(res.a.b.a);  zero(res.a.b.b); 
return res;
}
if(mr_compare(tmp1,zero1)<0)
{
